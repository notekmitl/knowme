...
- Mobile `390x844`, three runs: authenticated POST 1/run, OPTIONS 0/1/0, Firestore `western_natal` reads 0, browser `astrology_fusion` traffic 0. Response-complete-to-readable median 87.985 ms / maximum 90.149 ms; click-to-readable median 1,251.186 ms / maximum 1,286.553 ms.
- Desktop `1535x863`, three runs: authenticated POST 1/run, OPTIONS 0/0/0, Firestore `western_natal` reads 0, browser `astrology_fusion` traffic 0. Response-complete-to-readable median 81.950 ms / maximum 82.005 ms; click-to-readable median 1,361.083 ms / maximum 2,953.287 ms.
...
- Mobile and desktop first-readable compositor frames rendered with no overflow, console overflow/error, or stuck loading text.
- The superseded 14.662 s mobile and 31.251 s desktop observations included Agent/tool wait and were not runtime measurements. The corrected trace uses click, POST dispatch, response headers, body completion, chart-state application, and the first matching compositor frame on one browser clock.

### Release identity and scope

Firebase Hosting release `sites/knowme-app-694e1/releases/1789977665171000` points to version `sites/knowme-app-694e1/versions/4065a55e03f5aa1e`. The deployed `main.dart.js` SHA-256 is `2c8f901f9858211603b4b3689aa04fe5e077c7a4866ca1a5dd83dc48b25b5335` with cache pin `de0a83b`. This final acceptance run changed no application source, build, Hosting release, Backend, IAM, Firestore, Functions, Auth, or Storage. Detailed methodology and all six accepted runs are recorded in `docs/WESTERN_ASTROLOGY_READER_V2_PERFORMANCE_CLOSEOUT.md`.
...
The Owner feedback that the prior screen did not feel like a reading is closed
in product code. After the existing `/beta/thai` birth form, the reader chooses
Thai, Chinese BaZi, or Western astrology. The Thai choice reuses the accepted
executor and original `startedAt`/Bangkok `asOf` contract. Chinese and Western
use one canonical authenticated profile handoff and prepare only the chosen
system before navigation.

Chinese BaZi now leads with a deterministic natal reading: overview, usable
strengths, work and roles, money and resources, relationships and shared space,
and watch-outs with a practical development prompt. The evidence line under
each area names the calculated Day Master and every joint-highest visible
relationship family/count used. The report then exposes Day Master detail,
relationship counts, input, pillars, visible elements, method, sources and
limitations. Web and PDF are projections of the same report object.

Known uses four pillars. Ordinary Unknown uses three and never persists the
Thai normalizer's noon sentinel. Western is disabled for Unknown time.
Li-Chun/Jie boundary-partial Unknown retains only invariant facts and Day Master
reflection; it omits the five whole-chart natal areas. A forced Western refresh
invalidates Fusion before generation, while the existing BaZi fingerprint gate
continues to close Known-to-Unknown leakage and stale-chart display.

Both new clients use Firebase Bearer tokens and versioned endpoints. Backend
v1 routes reject UID mismatch and write only the verified UID. Legacy paths are
explicit for rollout compatibility; the old BaZi route remains authenticated,
and the old unauthenticated Western route is not used by the new client. Nothing
was deployed.

Final gates: backend **22/22**, focused Flutter **79/79**, full Flutter
**3,070/3,070**, analyzer 282 inherited non-fatal diagnostics / scoped **0**,
release Web build PASS, actual loopback URL count **0**, and
`product-acceptance/` delta **0**. Twenty-two tracked outputs regenerated by the
full suite were restored by exact path. Thai calculation/report/goldens have no
delta.

PDF QA covers 15/15 freshly rendered A4 pages: Known 5, ordinary Unknown 4,
Li-Chun Unknown 3, Jie Unknown 3. Fonts are
`NotoSansThai-Regular/Bold` plus `NotoSansSC-Regular`; text extraction,
...
KnowMe now has a completed Chinese astrology V1 report built on BaZi/Four
Pillars. In addition to the governed calculation, the result includes a
deterministic symbolic reading for all ten Day Masters, constructive reflection,
one balance point and one practical prompt, plus five visible-element
relationship families. Web, plain text and PDF use the same report model and
print the calculation contract, reading contract, input fingerprint, sources
and limitations.

This layer is source-governed rather than generated ad hoc. The calculation
ledger names Hong Kong Observatory material and the pinned
`lunar-python@1.4.8` implementation; the interpretation ledger names the Ten
Day Masters and elemental relationship framework. KnowMe's Thai/English copy is
an authored paraphrase for self-reflection, not scientific proof or a promised
future outcome.

Known uses all four pillars. Ordinary Unknown excludes Hour and its two visible
element slots. Li Chun/Jie Unknown keeps the invariant Day Master reading but
omits a chart-wide relationship emphasis. Tests pin all ten Day Masters, all
five element-cycle mappings, forbidden promise language, stale-chart behavior,
Fusion Known-to-Unknown invalidation and Web/PDF parity.

Final results on Flutter 3.41.1 / Dart 3.11.0 with `CI=true`, analytics
suppressed and `TZ=Asia/Bangkok`: backend **18/18**, Flutter focused **57/57**,
full Flutter **3,055/3,055**, analyzer exit 0, 282 inherited non-fatal
diagnostics and scoped findings **0**. Twenty-two tracked outputs regenerated by
the full suite were restored by exact path. Thai source/golden and
`product-acceptance/` deltas are 0.

Four three-page A4 PDFs passed visual inspection across all 12 latest pages and
structural checks for text on every page, embedded Regular CJK font and
Known/Unknown omission rules. The Web release bundle passes route and Production
API checks at 8,469,824 bytes / SHA-256
`3DFF3095890C8EF72E00F499AA44EC6BFA7292CF0F02B548B439699E40C5FCE8`.
There are no loopback endpoints. One inherited `localhost` hostname comparison
remains a separate pre-Production strict-policy blocker.

No calculation or interpretation choice remains open for V1. Draft PR #122 is
the Owner-testing handoff. It remains stacked on Draft PR #120, and this result
does not authorize Ready for Review, merge, deployment, Production data access
or Firebase-service mutation.
...
Owner manual QA is explicitly limited to Known, ordinary Unknown, Li Chun
Unknown, Jie Unknown and Web/PDF parity. The fixture route is no-write and does
not exercise authentication. Token/UID/revoked-token behavior, regeneration and
Fusion freshness are automated engineering gates, not Owner fixture testing.
...
comparison. Actual loopback endpoints are 0; the literal comparison is not an
endpoint. Guard refinement is a future shared task and must close before
Production under policy, without changing shared runtime in PR #122.

The validated application remains commit
`8fe3c68e2c60ec9a1511e75bc22982a1d854c007`, tree
`a478defae8cd8f88435e8f5fda7c908db4a11773`. The earlier client-first then
immediate backend-enforcement plan is withdrawn because cached/open legacy
clients remain exposed. Release is blocked pending a separately implemented
authenticated versioned endpoint beside the legacy endpoint, migration of the
new client to it, adoption verification and only then legacy retirement. No
release, Ready transition, merge or deploy is authorized by this closeout.

This clarification is Markdown-only. Its docs-required PreCommit reverified the
eight-file scope, forbidden text, `git diff --check`, application identity and
the unchanged Web/PDF hashes; application tests, PDF generation and Web build
were not rerun.
...
Eight backend calculation assertions pass against `lunar_python==1.4.8` with a
documented Windows `ZoneInfo` stub. Chinese-focused Flutter tests pass **83/83**.
Analyzer exits 0 under repository policy with 297 existing warning/info
diagnostics. The full Flutter invocation reports **2,989 passed / 40 failed**;
the failures are existing Thai screenshot-golden pixel comparisons on Flutter
3.41.3, while the recorded 3,029/3,029 baseline used Flutter 3.41.1. No golden
was updated and there is no Thai/UI source delta.

Full evidence, limitations and the proposed non-predictive V1 boundary are in
