...
| **Planet Library attributes + attribute ontology (D-072 / Batch 8)** | `docs/THAI_CANON_KNOWLEDGE_PRODUCTION_BATCH_8.md` — D-072 adds 11 `attributeCategory` entities (+ `attribute` value tokens during extraction). pp.30–36: +293 `relates_to` attribute units; coverage 56→349. Commodity/physique deferred; OCR gaps on Venus scalars / Saturn gender. Ontology expansion committed separately |
| **Planet directions + attribute gaps (Batch 9)** | `docs/THAI_CANON_KNOWLEDGE_PRODUCTION_BATCH_9.md` — p37 **ครองทิศ** directions (+8; 349→357). Body part / season not produced (modeling gap / absent from source). Doc-only deferred note: `relates_to` may later specialize to typed attribute relations — not a current platform task |
