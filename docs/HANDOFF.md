...
**Important:** Prefer `main`. Public Firebase Hosting was last deployed from `main` @ `5529264` (2026-07-24) — includes Thai Beta **Life Map V1.2.6** past life-breadth (PR #35/#36). **V1.2.7** adds a deterministic simulated **864-profile** Life Map matrix (8 weekday categories × ages 1–108) with an independent oracle and optional `asOf` plumbing — see [`THAI_LIFE_MAP_V127_SIMULATED_MATRIX.md`](THAI_LIFE_MAP_V127_SIMULATED_MATRIX.md). Anonymous Public Beta: https://knowme-app-694e1.web.app/beta/thai (Evidence Badge remains `invited_beta`). Validation status: **Ready for Invited Beta Validation**.
