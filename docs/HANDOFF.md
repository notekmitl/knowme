...
**Important:** Prefer `main`. Public Firebase Hosting was last deployed from `main` @ `07d0eb9` (2026-07-23) — Thai Beta Report **Life Map V1.2.3** + **Mahabhut Canon-index fix** (PR #20) + anonymous Public Beta at https://knowme-app-694e1.web.app/beta/thai?v=07d0eb9 (Thai landing, not Login; Evidence Badge remains `invited_beta`). **V1.2.4 Accuracy Audit** is on `main` @ `cb33a3d` (tests/docs). **V1.2.5** adds invite-only Life Map feedback (`invited_beta_testers` + `thai_life_map_beta_feedback`) — status **Ready for Invited Beta Validation**; see [`THAI_LIFE_MAP_V125_BETA_VALIDATION.md`](THAI_LIFE_MAP_V125_BETA_VALIDATION.md). Older feature branches such as `feature/fusion-result` remain useful historical snapshots; check [`CURRENT_STATUS.md`](CURRENT_STATUS.md) for the active deploy source.
