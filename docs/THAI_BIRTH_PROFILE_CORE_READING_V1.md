...
Round 9 changes only future forecast typed decision composition, profile-aware acceptance pairing, controlled production mutation validation, and exported forecast prose. Core Reading ownership, evidence meaning, day-boundary rules, known-time calculations, and unknown-time omissions are unchanged. Rounds 5–8 compatibility notes are historical and superseded here. Product Acceptance passed as `PRODUCT_ACCEPTANCE_PASS — ROUND 9 FIX SET 03` on 2026-08-10; merge and Production deployment remain pending final integration.
