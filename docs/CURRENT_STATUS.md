...
**Thai Mirror QA screenshot baseline:** Repair V1 is in isolated validation from unchanged `origin/main` (`fdf2cb3ccdbdacb8cd33bd0c41e6ff1fe35fe2d8`) with Flutter 3.41.1. The stale baseline failed all 24 profile/viewport cases before regeneration; 51 harness PNGs were refreshed and repeat runs are deterministic. The separate standalone consumer golden remains a known 32.63% / 305,379-pixel failure and requires its own task. No production source or Thai Beta V3 code is in scope.
