...
|----------------|---------------|----------|----------|-------|
| Birth Normalization | `lib/features/birth_normalization/` | `RawBirthInput` (date/time/province/country/timezone/coords) | Deterministic `NormalizedBirth` — resolved location/timezone/calendar, location-/season-/timezone-aware local sunrise, per-system contexts (Thai sunrise-based astrological date · Western exact instant · BaZi placeholder) + traceable `BirthNormalizationReason`s | Active (foundation; the single birth-input layer before every engine; engine migration to consume it is documented follow-up) |
| Western Natal | astrology services + `astrology/western_natal` | Birth profile (→ `NormalizedBirth` target) | Natal chart | Frozen V1 |
...
| Thai Knowledge Foundation | `lib/features/astrology/thai/knowledge/` | Frozen engine rules (read-only) — V1: `PlanetRelationshipMatrix` | One evidence record per rule (source school/name/reference/page, confidence, verified, notes) + coverage report; values mirror the frozen rules (no drift) | Active V1 (Planet Relationship only; read-only; no engine change; all `Unknown`/`verified=false` today) |
...
| Thai Mirror Conversation Experience | `lib/features/astrology/thai/conversation/` | A `ConversationSession` (chart anchors + state + memory) + a selected `ConversationQuestion` id from the fixed `ConversationCatalog` | Deterministic next `ConversationSession`: a `ConversationAnswer` (structured V13 runtime output) + `ConversationSuggestion`s (curated follow-ups → next selection), via `ConversationFlow.openTopic`/`ask` | Active V16 (experience foundation; consumes the V13 runtime only; no AI, no LLM, no chat model, no parser, no free text, no UI) |
