...
async function runFixture(browser, baseUrl, outputRoot, executableLabel, fixtureId) {
  ...
  try {
    ...
    await page.waitForTimeout(300);
    await screenshot(page, outputRoot, executableLabel, fixtureId, 'mobile-390-top');
    await page.mouse.move(360, 760);
    for (let index = 0; index < 8; index += 1) {
      await page.mouse.wheel(0, 620);
      await page.waitForTimeout(80);
    }
    await screenshot(page, outputRoot, executableLabel, fixtureId, 'mobile-390-middle');
    for (let index = 0; index < 32; index += 1) {
      await page.mouse.wheel(0, 620);
      await page.waitForTimeout(80);
    }
    await screenshot(page, outputRoot, executableLabel, fixtureId, 'mobile-390-bottom');
    result.mobileState = await page.evaluate(() => ({
      viewport: { width: innerWidth, height: innerHeight, dpr: devicePixelRatio },
