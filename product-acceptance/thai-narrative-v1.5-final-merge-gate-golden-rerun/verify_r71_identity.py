...
folder_sums = parse_sums((R71 / "SHA256SUMS.txt").read_text(encoding="utf-8"))
folder_mismatches = [
    path
    for path, expected in folder_sums.items()
    if not (R71 / path).is_file() or sha256_file(R71 / path) != expected
]
...
for item in comparison["files"]:
    relative = item["path"]
    r7_path = R7 / relative
    r71_path = R71 / relative
    if (
        not r7_path.is_file()
        or not r71_path.is_file()
        or sha256_file(r7_path) != item["r7Sha256"]
        or sha256_file(r71_path) != item["r71Sha256"]
        or sha256_file(r7_path) != sha256_file(r71_path)
    ):
...
lines = [
    ...
    f"folderChecksumEntries: {len(folder_sums)}",
    f"folderChecksumMismatches: {len(folder_mismatches)}",
