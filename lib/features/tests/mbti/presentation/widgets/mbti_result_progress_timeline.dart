...
/// Visual 16 → 40 → 80 checkpoint timeline (presentation only).
class MbtiResultProgressTimeline extends StatelessWidget {
  const MbtiResultProgressTimeline({super.key, required this.scoredQuestionCount});
  ...
  static _TimelineNodeState _stateFor(int checkpoint, int scored) {
    ...
    final nextTarget = scored < mbtiMiniCheckpoint
        ? mbtiMiniCheckpoint
        : scored < mbtiStandardCheckpoint
            ? mbtiStandardCheckpoint
            : scored < mbtiAccurateCheckpoint
                ? mbtiAccurateCheckpoint
                : 0;
  ...
  Color _tierColor(int checkpoint) {
    if (checkpoint == mbtiMiniCheckpoint) return Colors.green.shade600;
    if (checkpoint == mbtiStandardCheckpoint) return Colors.amber.shade700;
    return Colors.deepPurple;
  }

  String _tierSubtitleKey(int checkpoint) {
    if (checkpoint == mbtiMiniCheckpoint) {
      return 'mbti_result_timeline_tier_mini';
    }
    if (checkpoint == mbtiStandardCheckpoint) {
      return 'mbti_result_timeline_tier_standard';
    }
    return 'mbti_result_timeline_tier_accurate';
  }

  @override
  Widget build(BuildContext context) {
    final scored = scoredQuestionCount.clamp(0, mbtiAccurateCheckpoint);
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Text(
              AppText.t('mbti_result_progress_title'),
            ...
            Text(
              AppText.t('mbti_result_progress_hint'),
              style: TextStyle(
                fontSize: 12,
                height: 1.25,
...
class _TimelineNode extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final activeColor = state == _TimelineNodeState.locked
        ? muted
        : tierColor;
    ...
    final circleDecoration = BoxDecoration(
      ...
      border: state == _TimelineNodeState.current
          ...
          : state == _TimelineNodeState.locked
              ...
              : null,
      boxShadow: state == _TimelineNodeState.current
          ? [
              BoxShadow(
      ...
      children: [
        ...
        Text(
          ...
          style: TextStyle(
            ...
            color: state == _TimelineNodeState.locked ? muted : activeColor,
          ),
        ),
        Text(
        ...
        Text(
          '$checkpoint ${AppText.t('mbti_result_checkpoint_questions')}',
          textAlign: TextAlign.center,
          style: TextStyle(
            fontSize: 10,
