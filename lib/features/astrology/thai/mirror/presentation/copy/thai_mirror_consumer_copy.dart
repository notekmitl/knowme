...
enum _HeadlinePattern { personLabel, decisionMoment, othersSee, priorityFocus }
...
abstract final class ThaiMirrorConsumerCopy {
  ...
  static String tagLabel(String themeId) =>
      ThaiMirrorThemePhrases.phrase(themeId).tag;
  ...
  static String enforceHeadlineLength(
    String text, {
    int minWords = 8,
    int maxWords = 15,
  }) {
  ...
  lifeAspectDashboardParts({
    ...
    final hintIndex =
        (profileSeed.abs() + aspectOffset * 3) %
    ...
    final whyItAppears = 'ดวงสะท้อน${phrase.tag} — ${phrase.heroDetail}';
  ...
  static String lifeAspectSummary({
    ...
    final growthThemeId = growthPathIds.isNotEmpty
        ? growthPathIds[(profileSeed.abs() + aspectOffset) %
              growthPathIds.length]
        : null;
    ...
    for (var i = 0; i < candidates.length; i++) {
      ...
      if (enriched.isNotEmpty && !usedSummaries.contains(enriched))
        return enriched;
    }
  ...
  static ThaiMirrorBirthDataConfidenceState birthDataConfidence({
    ...
    // V1.3.2: complete data = silent. Incomplete limitation is folded into
    // hero.identitySubtitle (birth-data metadata), not a mid-report banner.
    if (hasBirthTime) {
