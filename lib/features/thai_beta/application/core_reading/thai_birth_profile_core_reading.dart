...
class ThaiBirthProfileCoreReading {
  ...
  static ({String analysis, String guidance}) _composeHouseDomain(
    ...
    final mode = _planetMode(lord.rawValue);
    final evidenceFrame = '${_lagnaLabel(sign.rawValue)}:';
    return switch (domain) {
      ThaiBirthProfileCoreDomain.work => (
        analysis:
            '$evidenceFrame เรื่องงาน เรือนนี้ชี้ว่าคุณสร้างผลงานผ่าน${mode.$1} จุดตัดสินไม่ได้อยู่ที่รับงานได้อีกเท่าไร แต่อยู่ที่บทบาทนั้นเพิ่มอำนาจกำหนดคุณภาพงานหรือเพียงเพิ่มน้ำหนักให้คุณถือ',
        guidance:
      ...
      ThaiBirthProfileCoreDomain.money => (
        analysis:
            '$evidenceFrame เรื่องเงิน รูปแบบของเรือนนี้ผูกความมั่นคงเข้ากับ${mode.$1} จึงวัดความก้าวหน้าได้จากทางเลือกที่เงินสำรองเปิดให้ ไม่ใช่เพียงยอดที่สะสม',
        guidance:
      ...
      ThaiBirthProfileCoreDomain.relationships => (
        analysis:
            '$evidenceFrame ในความสัมพันธ์ ความไว้ใจของคุณเกิดผ่าน${mode.$1} ความสัมพันธ์จึงลึกขึ้นเมื่อความรับผิดชอบที่แบ่งกันสอดคล้องกับพื้นที่ที่แต่ละฝ่ายต้องมี',
        guidance:
      ...
      ThaiBirthProfileCoreDomain.wellbeing => (
        analysis:
            '$evidenceFrame เรื่องพลังชีวิต เรือนนี้เชื่อมความสม่ำเสมอของร่างกายกับ${mode.$1} สัญญาณสำคัญจึงไม่ใช่วันที่ล้าเพียงวันเดียว แต่คือเวลาฟื้นตัวที่ยาวขึ้นหลังช่วงรับผิดชอบหนัก',
        guidance:
